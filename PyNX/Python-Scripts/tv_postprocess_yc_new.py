...
def slice_center(arr, new_shape):
    ...
    - new_shape: A tuple (x, y, z) specifying the size of the new slice.
...
def phasecor(data):
    a_angle = np.angle(data[np.abs(data)>5e-4]).mean()
    print("Average angle ", a_angle)#*180/np.pi)
    if np.abs(a_angle) < 0.1:
        print("Correct the phase")
        #data = (data.imag-1j*data.real)*np.exp(-1j*(a_angle-0.785))#np.exp(1j*0.785)
        data = data*np.exp(1j*0.785)
        #data = data*np.exp(1j*1.57)
    a_angle = np.angle(data[np.abs(data)>5e-4]).mean()
    print("Average angle ", a_angle)#*180/np.pi)
    return data
...
def load_rec(filename,size):
    ...
    #data = zoom(data,768/560,order=1)
    print(data.dtype)
    #data = phasecor(data)#correct for the phase
    shape = data.shape
    if size>np.array(shape).max():
        print("Reconstruction is smaller than array convert to size "+str(size))
        ndata = np.zeros((size,size,size),data.dtype)
        ndata[size//2-shape[0]//2:size//2-shape[0]//2+shape[0],size//2-shape[1]//2:size//2-shape[1]//2+shape[1],size//2-shape[2]//2:size//2-shape[2]//2+shape[2]] = data + 0
...
def load_data(filename,size):
    ...
    s2 = shape[1]//2
    s3 = shape[2]//2
    #data[s1-7:s1+7,s2-7:s2+7,s3-7:s3+7]=0
    ...
    if size<np.array(shape).max():
        ...
        ndata = np.zeros((size,size,size),data.dtype)
        ndata = data[shape[0]//2-size//2:shape[0]//2+size//2,shape[1]//2-size//2:shape[1]//2+size//2,shape[2]//2-size//2:shape[2]//2+size//2] + 0
        #ndata = np.zeros((size,size),data.dtype)
        #ndata = data[shape[0]//2-size//2:shape[0]//2+size//2,shape[1]//2-size//2:shape[1]//2+size//2] + 0
        del data
        ndata[ndata>0] = 1
        ndata[np.isnan(ndata)]=0
        return ndata.astype(bool)
    elif size>np.array(shape).max():
        print("Diffraction volume is smaller than array convert to size "+str(size))
        ndata = np.zeros((size,size,size),data.dtype)
        ndata[size//2-shape[0]//2:size//2+shape[0]//2,size//2-shape[1]//2:size//2+shape[1]//2,size//2-shape[2]//2:size//2+shape[2]//2] = data + 0
        del data 
        ndata[ndata>0] = 1
        ndata[np.isnan(ndata)]=0
        return ndata.astype(bool)
...
def main():
    ...
    #Run TV
    u, costs = solve_tv_problem(u, xp_fft.fftn(u,workers=ww), mask, stepsize, nits, verbose)
    #u2, costs = solve_tv_problem(u, xp_fft.fftn(u), mask, 30*stepsize, int(0.2*nits), verbose)
    #u2, costs = solve_tv_problem(u2, xp_fft.fftn(u2), mask, 10*stepsize, int(0.2*nits), verbose)
    #u2, costs = solve_tv_problem(u2, xp_fft.fftn(u2), mask, stepsize, nits, verbose)
